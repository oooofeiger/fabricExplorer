...
@connect(({ network }) => {
  ...
  class DeployPeer extends React.Component{
    ...
    handleSubmit = () => {
        ...
        form.validateFields((err, values)=>{
            if(!err){
                ...
                value.chaincodePort = values.chaincodePort_;
    ...
    render(){
        const { network, form, peerImageVersion } = this.props;
        const { ordererConfig, couchdbConfig, peerConfig } = network;
        ...
        console.log('deployPeerState',this.state,'deployPeerProps',this.props);
            ...
            let notDeployPeer = [];
            if(peerConfig){
                notDeployPeer = peerConfig.filter((item,i)=>{
                    return item.isDeploy === false
                })
            }
        ...
        return(
            <Form onSubmit={this.handleSubmit}>
                ...
                >
                    {
                        getFieldDecorator('port',{
                            initialValue: '22',
                            rules: [{
                                required: true,message: '请输入节点请求端口！'
                            },{
                ...
                >
                    {
                        getFieldDecorator('userName',{
                            initialValue: '',
                            rules: [{
                                required: true,message: '请输入部署服务器的用户名！'
                            }]
                        })(
                            <Input  />
                        )
                ...
                >
                    {
                        getFieldDecorator('password',{
                            initialValue: '',
                            rules: [{
                                required: true,message: '请输入部署服务器的密码！'
                            }]
                        })(
                            <Input type="password"  />
                        )
                ...
                >
                    {
                        getFieldDecorator('composeFileFormat',{
                            initialValue: '',
                            rules: [{
                                required: true,message: '请输入docker-compose版本！'
                            }]
                        })(
                            <Input  />
                        )
                ...
                >
                    {
                        getFieldDecorator('ordererName',{
                            initialValue: '',
                            rules: [{
                                required: true,message: 'orderer不能为空！'
                            }]
                        })(
                            <Select
                                placeholder="选择一个orderer"
                            >
                                {
                ...
                >
                    {
                        getFieldDecorator('couchdbName',{
                            initialValue: '',
                            rules: [{
                                required: this.state.useCouchdb,message: 'couchdb不能为空！'
                            }]
                        })(
                            <Select
                                placeholder="选择一个couchdb"
                                disabled={!this.state.useCouchdb}
                            >
                                {
                                    couchdbConfig && couchdbConfig.map((item,i)=>{
                                        return (<Option key={i} value={item.couchdbName}>{item.couchdbName}</Option>)
                                    })
                                }
                            </Select>
                        )
                ...
                >
                    {
                        getFieldDecorator('peerName',{
                            initialValue: '',
                            rules: [{
                                required: true,message: 'peer节点不能为空！'
                            }]
                        })(
                            <Select
                                placeholder="选择一个peer节点"
                            >
                                {
                                    notDeployPeer && notDeployPeer.map((item,i)=>{
                                        return (<Option key={i} value={item.peerName}>{item.peerName}</Option>)
                                    })
                                }
                            </Select>
                        )
                ...
                >
                    <Button type="primary" block htmlType="submit">部署</Button>
                </FormItem>
            </Form>
        )
    }
