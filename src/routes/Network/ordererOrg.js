...
@connect(({ network, loading }) => {
 ...
 class OrdererOrgNetwork extends React.Component {
     ...
     getCertId = () => {
         ...
         form.validateFields(['name','mspId','caHost','generateCert'],(err,values)=>{
             if(!err){
                dispatch({
    ...
    download = (name) => {
        ...
        dispatch({
            type: 'network/ordererOrgDownload',
            payload: {
                name,
    ...
    delete = (name) => {
        const { dispatch, sessionId } = this.props;
        if(confirm('删除现有Orderer组织的配置信息，与之关联的orderer节点信息也将被一并删除，继续删除吗？')){
            dispatch({
    ...
    render(){
        ...
        const { ordererOrgConfig, ordererOrgCertId, peerOrgDownload, peerOrgDelete, ordererOrgGet } = network;
                        ...
                        <Row gutter={24}>
                            <Col md={24}>
                                <div className={styles.blockListTable}>
                                    ...
                                    <Table loading={loading} pagination={{pageSize:10}} bordered dataSource={ordererOrgConfig?[ordererOrgConfig]:null} columns={ordererOrgConfigCol} />
                                </div>
                            </Col>
                        </Row>
                    </TabPane>
                                    ...
                                    <Form onSubmit={this.handleSubmit}>
                                        ...
                                        >
                                            {
                                                getFieldDecorator('name',{
                                                    initialValue:addSet?'':ordererOrgGet.name,
                                                    rules: [{
                                        ...
                                        >
                                            {
                                                getFieldDecorator('mspId',{
                                                    initialValue:addSet?'':ordererOrgGet.mspId,
                                                    rules: [{
                                        ...
                                        >
                                            {
                                                getFieldDecorator('caHost',{
                                                    initialValue:addSet?'':ordererOrgGet.caHost,
                                                    rules: [{
                                        ...
                                        >
                                            {
                                                getFieldDecorator('generateCert',{
                                                    initialValue:addSet?true:ordererOrgGet.generateCert,
                                                })(
                                        ...
                                        >
                                            <Row gutter={8}>
                                                ...
                                                {getFieldDecorator('certId',{
                                                    initialValue: ordererOrgCertId ? ordererOrgCertId.certId : '',
                                                    rules: [{
                                        ...
                                        >
                                            ...
                                            rules:[{required: true}]
                                                ...
                                                <Upload  
                                                disabled={ordererOrgCertId?false:true} 
                                                name="file" 
                                                fileList={fileList}
                                                action={`http://${host}/network/peerOrg/cert/upload/${ordererOrgCertId?ordererOrgCertId.certId:''}`}
